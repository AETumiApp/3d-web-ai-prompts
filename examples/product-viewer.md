...
light it well, and keep it accessible and fast. Fill the brackets and send.
...
- Frame the model automatically: compute its bounding box, centre it at the
  origin, and position the camera to fit it in view regardless of the model's
  authored scale.
- Lighting: an environment map or a simple 3-point rig (key + fill + rim) so
  metals and highlights read correctly. Use ACESFilmicToneMapping and
  SRGBColorSpace output.

ARCHITECTURE
- Client Component ('use client'); three mounted in useEffect via dynamic
...
- Provide a keyboard-accessible fallback control (e.g. buttons to rotate
  left/right/reset) so the product is inspectable without a mouse drag.
...
- Handle resize; handle a failed model load gracefully (show the poster + a
  short message, don't white-screen).
- Full cleanup on unmount: dispose the loaded scene's geometries, materials and
  textures (traverse and dispose), dispose controls, renderer and environment
  map, cancel any RAF, remove listeners, remove the canvas.
...
- Ask for the auto-framing (bounding-box fit) explicitly — models come in wildly
  different scales and this is the most common "why is it tiny / clipped" bug.
- If you have an HDRI, name it so the assistant wires up `RGBELoader` for the
  environment map instead of hand-building lights.
